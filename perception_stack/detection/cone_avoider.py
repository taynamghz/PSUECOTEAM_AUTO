...
def _clearance_target(gl: float, gr: float, ref_X: float = 0.0) -> float:
    """
    Closest X to ref_X inside [gl, gr] that keeps the car clear of both edges.
    ref_X should be the lane centerline X in camera frame (= deviation_m).
    ...
    if lo >= hi:
        return (gl + gr) / 2.0
    return float(np.clip(ref_X, lo, hi))
...
def _find_best_gap(
    ...
    left_X_m: float,
    right_X_m: float,
    lane_center_X: float = 0.0,   # lane centerline X in camera frame (deviation_m)
) -> float:
    """
    Best gap clearance-target X through the (priority) cone field.
    Prefers gaps that keep the car close to the lane centerline (minimum deviation).
    ...
    for gl, gr, ct in gaps:
        ...
        if abs(gr - safe_right) < 0.05:
            edge_penalty = track_span * 0.6
        # Deviation penalty relative to lane centerline (not camera centre)

...
def draw(frame: np.ndarray, result: PerceptionResult,
    ...
    # ── Cone avoidance debug overlay ──────────────────────────────────────────
    # When cone_avoider is available: full debug (gap bands, lane EMA lines,
    # per-cone X/Z labels, raw vs EMA target lines, HUD).
    # Fallback: simple bbox + gap waypoint marker from PerceptionResult.
    if cone_avoider is not None:
        vis = cone_avoider.debug_overlay(vis, H=H, W=W, fx=fx)
        if result.avoidance_state == "AVOIDING" and result.lookahead_pixel is not None:
            gx, gy = result.lookahead_pixel
            cv2.drawMarker(vis, (gx, gy), (0, 255, 80), cv2.MARKER_STAR, 22, 2)
    else:
